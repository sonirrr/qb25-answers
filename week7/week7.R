...
order_kmeans_results <- kmeans_results$cluster[order(kmeans_results$cluster)]
order_combined <- combined[order(kmeans_results$cluster), ]
...
cluster2genes <- write.table(rownames(cluster2))
writeLines(rownames(cluster2), "cluster2.txt")
cluster3genes <- rownames(cluster3)
cat(cluster3genes, sep='\n')
cluster5genes <- rownames(cluster5)
cat(cluster5genes, sep = '\n')
cluster7genes <- rownames(cluster7)
cat(cluster7genes, sep = '\n')
